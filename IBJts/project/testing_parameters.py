...
if __name__ == '__main__':
    ...
    with open('results2.txt') as f:
        for line in f.readlines():
            ...
            if words[0] == "Highest":
                if words[1] == "VWR:":
                    parameters.append([float(words[4]), int(words[5]), int(words[6]), int(words[7])])
                else:
                    parameters.append([float(words[5]), int(words[6]), int(words[7]), int(words[8])])
